...
#' Function to process list of filenames, using GGIR output located in "output_dir"
#' 
#' @param file_ls a vector of file names to process, not including path. These are 
#' specifically for GENEActiv watch '.bin' data
...
#'
#' @return data.table containing extracted features for each file in file_ls, as 
#' indicated by "filename" column
#' 
#' #' @example dt <- process_file_ls(c("patient_0.bin", "patient_1.bin"), "/Desktop/GGIR_output")
process_file_ls <- function(file_ls, output_dir) {
  n <- length(file_ls)
  dt <- data.table(filename = as.character(NA),
                   temp.allnight.entropy = as.numeric(NA),
  ...
  dt <- rbindlist(rep(list(dt), n))
  for (i in 1:length(file_ls)) {
    filename <- file_ls[i]
    feature_output_ls <- get_temp_features(filename, output_dir)
    night.table <- feature_output_ls[[1]]
    allnights <- feature_output_ls[[2]]
    
    # assign all features to row
    dt$filename[[i]] <- filename
